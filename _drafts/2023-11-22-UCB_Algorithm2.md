...
As we will work with a sequence of random variables, I need the following definition to be more general.
...
Note that the distribution of $X_t$ may change depending on $X_1, X_2, ..., X_{t-1}$, but its mean and tail probability should still satisfy the property of a subGaussian random variable.
...
Note that $X_t$ may depend on all the previous action selections including the $t$-th one and observed rewards, but it must still satisfy the property of $\sigma^2$-subGaussian at the point of being sampled, hence "conditionally" $\sigma^2$-subGaussian.
...
Of course, if the sums $Y_{t, k}$s are bounded by the Azuma inequality, the proof of the UCB algorithm works.
The question is why can't we just take the subsets of the sums and take union bound over $T$ sums?
...
> $$
*Proof* : I think the whole proof deserves a separate post, as it includes many useful concepts.
